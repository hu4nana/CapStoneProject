...
public class Manuka_Gun : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.layer == 9)
        {
            ...
            MonsterAttack ma = other.gameObject.GetComponent<MonsterAttack>();

            if (ma == null)
            {
                ma = other.gameObject.GetComponentInParent<MonsterAttack>();
            }
            damagae = (int)ma.Damage;

            g_curHP -= damagae;
            Debug.Log($"플레이어의 현재 체력 : {g_curHP}");
            if (g_curHP <= 0)
            {
                g_curHP = 0;
                PlayerDie();
    ...
    {
        Time.timeScale = 0;
        GameObject.Find("Canvas").GetComponent<UIManager>().GameOver_Panel_On();
    }
}
